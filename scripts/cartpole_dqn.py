...
from toilet_cleaning_dqn import ToiletCleaning
...
class QNetwork:
    def __init__(self, learning_rate=0.01, state_size=13, action_size=3, hidden_size=20):
...
class Actor:
    def get_action(self, state, episode, targetQN):   # [C]ｔ＋１での行動を返す
        ...
        if epsilon <= np.random.uniform(0, 1):
            ...
            action = np.random.choice([0, 1, 2])  # action randomly
...
if __name__=='__main__':
    ...
    for episode in range(NUM_EPISODES):
        ...
        # get done and state from Happy Burger in initial
        ...
        for t in range(MAX_STEPS + 1):   
            ...
            is_acted = tc.doAction(action)
            if is_acted == False:
                reward = -1
            else:
                reward = tc.checkReward()
            # get reward and state from Happy Burger
